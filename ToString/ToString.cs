...
namespace Vsite.CSharp.ZajedničkaSvojstvaTipova
{
	struct KompleksniBroj
	{
		...
		{
			string realni = Realni == 0 ? "" : Imaginarni > 0 ? $"{Realni}+" : $"{Realni}";
			string imaginarni = Imaginarni == 0 ? "" : Imaginarni == -1 ? "-j" : Imaginarni == 1 ? "j" : $"{Imaginarni}j";
			return realni == "" && imaginarni == "" ? "0" : realni + imaginarni;
		}
