    ...
    struct KompleksniBroj
    {
        ...
        public override string ToString()
        {
            if (Realni != 0 && Imaginarni > 0)
                return Realni.ToString() + "+" + ((Imaginarni == 1.0) ? "" : Imaginarni.ToString()) + "j";
            if (Realni != 0 && Imaginarni < 0)
                return Realni.ToString() + "-" + ((Imaginarni == -1.0) ? "" : (-1 * Imaginarni).ToString()) + "j";
            if (Imaginarni != 0)
                return ((Imaginarni == 1) ? "" : ((Imaginarni == -1) ? "-" : Imaginarni.ToString())) + "j";
